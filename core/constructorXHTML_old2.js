...
function escaparHTML(texto) {
    if (!texto) return '';
    return String(texto)
        // 🚀 FILTRO SANITARIO: Elimina caracteres invisibles de InDesign
...
function procesarTextoInterno(token) {
    ...
    // 🚀 NORMALIZADOR QUIRÚRGICO DE ARTÍCULOS (Corrige el formato sucio de InDesign)
...
function construirEstructura(tokensEntrada, documentoEntrada, nombreCSS = 'Lexdigital_Modular.css', cssCrudoOriginal = '') {
    ...
    tokens.forEach((token) => {
        ...
        const clase = normalizarClase(token.claseLegal || token.estiloParrafo || token.tipo || 'parrafo');
