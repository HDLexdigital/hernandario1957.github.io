...
function processDok(args) {
    ...
    if (px.processText) {
        var noParStyleID = dok.paragraphStyles[0].id;
        var findProperties = {
            findWhat: "(?s).+"
        }
        ...
        for (var i = 0; i < results.length; i++) {
            var text = results[i];
            if (px.ignoreTextWithoutStyle ) {
                for (var j = 0; j < text.paragraphs.length; j++) {
                    if (text.paragraphs[j].appliedParagraphStyle.id == noParStyleID) {
                        continue;
                    }
                    text.paragraphs[j].clearOverrides();
                }
            }
            else {
                text.clearOverrides();
...
function getConfig(dok, px) {
    ...
    var ignoreTextWithoutStyleCheckBox = textPanel.add("Checkbox", undefined, "Ignore text ranges without an assigned paragraph style");
    ignoreTextWithoutStyleCheckBox.value = px.ignoreTextWithoutStyle;
    ignoreTextWithoutStyleCheckBox.enabled = processTextCheckBox.value;
    ...
    okButton.onClick = function () {
        ...
        px.ignoreTextWithoutStyle = ignoreTextWithoutStyleCheckBox.value;
