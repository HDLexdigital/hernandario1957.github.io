...
function extraerNovedades(catalogo, manifestDir) {
    ...
    for (const doc of catalogo) {
        ...
        (doc.versions || []).forEach(versionId => {
            ...
            const createdAt = versionManifest?.createdAt || manifest?.createdAt || null;

            items.push({
                documentId: doc.documentId,
                title: versionManifest?.title || manifest?.title || doc.documentId,
                versionId,
                createdAt,
...
function main() {
    ...
    if (!catalogo) {
        console.error('❌ No se encontró el catálogo en ' + CATALOGO_PATH);
        process.exit(1);
    }
